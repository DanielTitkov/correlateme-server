...
func (sq *ScaleQuery) QueryIndicators() *IndicatorQuery {
	query := &IndicatorQuery{config: sq.config}
...
// Returns a *NotSingularError when exactly one Scale entity is not found.
...
// Returns a *NotSingularError when exactly one Scale ID is not found.
...
func (sq *ScaleQuery) Clone() *ScaleQuery {
	...
	return &ScaleQuery{
		...
		withIndicators: sq.withIndicators.Clone(),
...
func (sq *ScaleQuery) sqlAll(ctx context.Context) ([]*Scale, error) {
	...
	if query := sq.withIndicators; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Scale)
		for i := range nodes {
		...
		query.Where(predicate.Indicator(func(s *sql.Selector) {
			s.Where(sql.InValues(scale.IndicatorsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.scale_indicators
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "scale_indicators" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "scale_indicators" returned %v for node %v`, *fk, n.ID)
			}
...
func (sq *ScaleQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if ps := sq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector, scale.ValidColumn)
			}
		}
	}
	return _spec
...
func (sq *ScaleQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	selector := builder.Select(t1.Columns(scale.Columns...)...).From(t1)
	if sq.sql != nil {
		selector = sq.sql
		selector.Select(selector.Columns(scale.Columns...)...)
	}
	for _, p := range sq.predicates {
		p(selector)
	}
	for _, p := range sq.order {
		p(selector, scale.ValidColumn)
	}
	if offset := sq.offset; offset != nil {
...
func (sgb *ScaleGroupBy) sqlQuery() *sql.Selector {
	...
	for _, fn := range sgb.fns {
		columns = append(columns, fn(selector, scale.ValidColumn))
	}
...
func (ss *ScaleSelect) sqlQuery() sql.Querier {
	selector := ss.sql
